...
const Easings = {
    ...
    easeInOutQuart: (x: number) => x < 0.5 ? 8 * x * x * x * x : 1 - 8 * (--x) * x * x * x,

    easeInQuint: (x: number) => x * x * x * x * x,
    easeOutQuint: (x: number) => 1 + (--x) * x * x * x * x,
    easeInOutQuint: (x: number) => x < 0.5 ? 16 * x * x * x * x * x : 1 + 16 * (--x) * x * x * x * x,
    ...
    easeInCirc: (x: number) => 1 - Math.sqrt(1 - x * x),
    easeOutCirc: (x: number) => Math.sqrt(1 - (--x) * x),
    easeInOutCirc: (x: number) => x < 0.5 ? (1 - Math.sqrt(1 - (2 * x) * (2 * x))) / 2 : (Math.sqrt(1 - (-2 * x + 2) * (-2 * x + 2)) + 1) / 2,
    ...
    cubicBezier(x1: number, y1: number, x2: number, y2: number) {
        return (x: number) => {
            ...
            return y1 * (1 - t) * (1 - t) * (1 - t) + 3 * y2 * (1 - t) * (1 - t) * t + 3 * y1 * (1 - t) * t * t + x2 * t * t * t;
        };
    }
} as const;
